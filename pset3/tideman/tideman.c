...
//Before sorting, we need to compare the votes of each candidate.
//To do so, we'll use pointers that will check who wins.
int comparator(const void *a, const void *b)
{
    pair *orderA = (pair *)a;
    pair *orderB = (pair *)b;
...
//We then use a mathematical matrix to validate the lock.
void validateLock(int j)
{
    if (j == 0)
    {
    ...
    //Here we verify the correctness of the matrix using recursion.
    validateLock(j - 1);
    ...
    //If we have reached the maximum rank, the lock is then canceled.
    if (r == j)
    {
        lock = false;
    }
